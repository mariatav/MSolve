...
namespace ISAAR.MSolve.PreProcessor.Elements
{
    public class Hexa8u8pWithStochasticMaterial : Hexa8u8p
    {
            ...
            for (int i1 = 0; i1 < iInt; i1++)
                for (int i2 = 0; i2 < iInt; i2++)
                    for (int i3 = 0; i3 < iInt; i3++)
                    {
                        ...
                        var e = ((Matrix2D<double>)((IStochasticContinuumMaterial3DState)materialStatesAtGaussPoints[iPos]).GetConstitutiveMatrix(GetStochasticPoints(i1, i2, i3)));
                        for (int j = 0; j < 6; j++)
                            for (int k = 0; k < 6; k++)
                                afE[iPos, j, k] = e[j, k];
        ...
        private double[] GetStochasticPoints(int iX, int iY, int iZ)
        {
            ...
            double Y = 0;
            double Z = 0;
            double minX = base.Nodes[0].X;
            double minY = base.Nodes[0].Y;
            double minZ = base.Nodes[0].Z;

            for (int i = 0; i < 8; i++)
            {
                minX = minX > base.Nodes[i].X ? base.Nodes[i].X : minX;
                minY = minY > base.Nodes[i].Y ? base.Nodes[i].Y : minY;
                minZ = minZ > base.Nodes[i].Z ? base.Nodes[i].Z : minZ;
                for (int j = i + 1; j < 8; j++)
                {
                    X = X < Math.Abs(base.Nodes[j].X - base.Nodes[i].X) ? Math.Abs(base.Nodes[j].X - base.Nodes[i].X) : X;
                    Y = Y < Math.Abs(base.Nodes[j].Y - base.Nodes[i].Y) ? Math.Abs(base.Nodes[j].Y - base.Nodes[i].Y) : Y;
                    Z = Z < Math.Abs(base.Nodes[j].Z - base.Nodes[i].Z) ? Math.Abs(base.Nodes[j].Z - base.Nodes[i].Z) : Z;
                }
            }
